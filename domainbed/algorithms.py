...
class PALSBASE(CLIP):
    def __init__(self, input_shape, num_classes, num_domains, hparams, sentence_prompt=False):
        super(PALSBASE, self).__init__(input_shape, num_classes, num_domains, hparams)
    ...
    def update(self, minibatches, unlabeled=None, mean_enc=False):
        # minibatches = [[domain_1], [domain_2], [domain_3]]
        all_x = [data[0].cuda().float() for data in minibatches]
        all_y = torch.cat([data[1].cuda().long() for data in minibatches])
        ...
        if True: #mean_enc:
            ...
            _new_domain_features = [feature.repeat_interleave(len(self.hparams['class_names']), dim=0) for feature in domain_features]
    ...
    def _get_text_features(self, domain_feature, coop=False):
        #  reshape domain_feature: [7, 16 * self.EMBEDDING_DIM] -> [7, 16, self.EMBEDDING_DIM]
        domain_feature = domain_feature.reshape(-1, self.hparams['num_domain_tokens'], self.EMBEDDING_DIM)
        #domain_feature = domain_feature.reshape(-1, domain_feature.shape[1], self.hparams['num_domain_tokens'], self.EMBEDDING_DIM)
    ...
    def predict(self, x):
        ...
        if True: #False:
            ...
            text_feature = self._get_text_features(mean_domain_feature)
            #mean_domain_feature = torch.mean(domain_feature, dim=0, keepdim=True)
            ...
            #mean_domain_feature = torch.mean(domain_feature, dim=0, keepdim=True).repeat_interleave(len(self.hparams['class_names']), dim=0)
            #mean_domain_feature = torch.mean(domain_feature, dim=0, keepdim=True)
            #domain_feature = (mean_domain_feature + domain_feature) / 2

            new_domain_feature = [feature.repeat_interleave(len(self.hparams['class_names']), dim=0) for feature in domain_feature]
