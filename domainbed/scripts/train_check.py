...
if __name__ == "__main__":
    ...
    parser.add_argument('--envs', type=str,
        help='JSON-serialized hparams dict',
        default="""{\"base_envs\": [[0],[1], [2], [3], [0, 1, 2, 3]],
            \"test_envs\": [0, 1, 2, 3]    
        }""")
    ...
    hparams['test_envs'] = [int(i) for i in env_dict["test_envs"]]
    ...
    def save_checkpoint(filename, args, algorithm):
        ...
        #print(algorithm.cpu().state_dict().keys())
        print("SAVE CHECKPOINT: %s" % filename)
    ...
    ####### SET UP EACH BASE AND ADAPTED ENVIRONMENT #########

    base_envs = env_dict["base_envs"]
    ...
    for base_env in base_envs:
        ...
        test_envs = env_dict["test_envs"]
        test_envs = [int(t) for t in test_envs if int(t) not in base_env]
        base_env_name = "_".join((str(b) for b in base_env))
